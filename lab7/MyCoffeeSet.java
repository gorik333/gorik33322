...
/**
 * Class MyCoffeeSet implements interface Set. This class describe data about object MyCoffeeSet.
 *
...
public class MyCoffeeSet implements Set<Coffee>{
	...
	public boolean add(Coffee coffee) {
		...
		if(size == element.length){
			System.out.println("Довжина колекції перед зміненням розміру:" + " " + element.length);
			resize();
			element[size] = coffee;
			System.out.println("Довжина колекції після змінення розміру:" + " " + element.length + "\n");
		}
		else
			element[size] = coffee;
		size++;
